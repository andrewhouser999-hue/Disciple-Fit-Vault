...
# Covenant Identity — Worked Session Examples — Index

*Fictional, composite client scenarios built to illustrate how a full CIC session actually runs — dialogue, coach-internal reasoning, and post-session documentation together, not just the abstract session-tool structure. Every scenario is built to be internally consistent with the live system documents it draws from (cited in `basis` above). No entry here is a real client record.*

*Each scenario is published as a companion Artifact (full script, easier to read end to end) and logged here with a one-line description and its pipeline stage. Add a new row each time a new scenario is built.*
...
*Related: [[Covenant Identity — Phase 1 Session Tools (Covenant Orientation)]] (the structural doctrine these examples dramatize), [[Covenant Identity — Discovery Call Guide]], [[Covenant Identity — Intake Addendum (Coach)]], [[AI Prompt — Designed Alliance]].*
